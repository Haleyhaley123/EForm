...
namespace EForm.Controllers.OPDControllers
{
    ...
    public class OPDHandOverCheckListController : EIOHandOverCheckListController
    {
        ...
        public IHttpActionResult GetHandOverCheckListAPI(Guid id)
        {
            ...
            var examination_note = opd.OPDOutpatientExaminationNote;
            if (examination_note == null || examination_note.IsDeleted)
                return Content(HttpStatusCode.NotFound, Message.OPD_OEN_NOT_FOUND);

            var diagnosis = examination_note.OPDOutpatientExaminationNoteDatas.FirstOrDefault(e => !e.IsDeleted && e.Code == "OPDOENDD0ANS");
        ...
        public IHttpActionResult GetPatientHandOverRecord(Guid id)
        {
            ...
            var clinical_examination_and_findings = new ClinicalExaminationAndFindings(clinicCode, clinic?.Data, oen.Id, opd.IsTelehealth, oen.Version).GetData();
