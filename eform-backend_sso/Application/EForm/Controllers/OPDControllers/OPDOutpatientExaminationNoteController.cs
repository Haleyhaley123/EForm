    ...
    public class OPDOutpatientExaminationNoteController : BaseOPDApiController
    {
        ...
        public IHttpActionResult GetExaminationNoteAPI(Guid id)
        {
            ...
            return Ok(new
            {
                ...
                oen.IsConsultation,
                IsUseHandOverCheckList = opd.OPDHandOverCheckList?.IsUseHandOverCheckList,
                IsAcceptPhysician = opd.OPDHandOverCheckList?.IsAcceptPhysician,
                IsAcceptNurse = opd.OPDHandOverCheckList?.IsAcceptNurse,
                ...
                IscheckVisitPreAnes = check_visit_PreAnes == null? false: true
            });
        }
        [HttpPost]
        [CSRFCheck]
        ...
        public IHttpActionResult UpdateExaminationNoteAPI(Guid id, [FromBody] JObject request)
        {
            ...
            var status_en = request["Status"]["EnName"].ToString();
            var visit_transfer = get_visit_transfer(opd);
            if (visit_transfer != null && status_en == "No examination")
            {
           ...
           // if (check != null && is_error == "OPDDC") return Content(HttpStatusCode.BadRequest, new { ViMessage = "Không thể đổi trạng thái do Người bệnh đã hoàn thành khám ở gây mê", EnMessage = "Không thể đổi trạng thái do Người bệnh đã hoàn thành khám ở gây mê" });
            if (is_error != null)
        ...
        public IHttpActionResult SyncVisitHistoryAPI([FromBody] JObject request)
        {
            ...
            return Content(HttpStatusCode.OK, new
            {
                PastMedicalHistory = visit_history_list.Where(e => !string.IsNullOrEmpty(e.PastMedicalHistory)).Select(e => new
                {
        ...
        private bool IsWaitingPrincipalTest(EDStatus status, DateTime? created_at)
        {
            ...
            return now > block_time &&
                ...
                status.EnName.Equals("Waiting results");
        }
        ...
        private void HandleUpdateAppointmentDateResult(OPDOutpatientExaminationNote oen, string appointment_date_result)
        {
            ...
            {
                DateTime request_appointmentDateResult = DateTime.ParseExact(appointment_date_result, Constant.TIME_DATE_FORMAT_WITHOUT_SECOND, null);
                if (appointment_date_result != null)
                {
        ...
        {
            if (!"OPDOENDORANS,OPDOENTD0ANS,OPDOENTOTANS".Contains(code) ||
                (code.Equals("OPDOENDORANS") && Validator.ValidateDate(value)) ||
        ...
        private dynamic HandleUpdateStatus(OPD opd, JToken request_status, JToken request_datas, bool isUseHandOverCheckList)
        {
            ...
            var new_status_en = request_status["EnName"].ToString();

            var err = IsChangeTransferedStatus(opd, request_datas, current_status, new_status_en);
            if (err != null)
                return err;
            ...
            if(check != null)
            {
                if(check.StatusId != null)
                {
                    ...
                    if (status.Code == "OPDIH" && new_status_en.Contains("In hospital")) return "OPDIH";
                    //if (status.Code == "OPDDC" && new_status_en.Contains("In hospital")) return "OPDDC";
            ...
            {
                if (current_status.EnName.Equals("Transfer to ED") || current_status.EnName.Equals("Admitted"))
                {
                    var visit_transfer = new VisitTransfer();
                ...
                if (new_status_en.Equals("In hospital") || new_status_en.Equals("Transfer to ED") || new_status_en.Equals("Admitted"))
                {
                ...
                if ((!new_status_en.Equals("Transfer to ED") && !new_status_en.Equals("Admitted")))
                    RemoveTransferIfExist(opd);

                var error_mess = OPDValiDatePIDAndVisitCode(opd, new_status_en);
                if (error_mess != null)
                    return error_mess;
            ...
            if ((new_status_en.Equals("Transfer to ED") || new_status_en.Equals("Admitted")))
            {
                var error = HandleTransferForAnotherDepartment(opd, request_datas, new_status_en, isUseHandOverCheckList);
                if (error != null)
                    return new
                    {
                        Code = HttpStatusCode.BadRequest,
        ...
        private dynamic OPDValiDatePIDAndVisitCode(OPD visit, string status_en)
        {
            if (status_en == "In hospital" || status_en == "Waiting results" || status_en == "No examination" || status_en == "Nonhospitalization")
                return null;
        ...
        private dynamic IsChangeTransferedStatus(OPD opd, JToken jToken, EDStatus current_status, string new_status_en)
        {
            string newStatus = new_status_en;
            string newTransfered = "";
            string oldStatus = current_status.EnName;
            string oldTransfered = "";
            var visit_transfer = get_visit_transfer(opd);
            if (newStatus.Equals("Transfer to ED"))
            {
            ...
            if (oldStatus.Equals("Transfer to ED"))
            {
            ...
            {
                if (oldStatus != newStatus) return visit_transfer;
        ...
        private dynamic HandleTransferForAnotherDepartment(OPD opd, JToken datas, string status_en, bool isUseHandOverCheckList)
        {
            Guid? receiving_id = null;
            string receiving = string.Empty;
            string reason = string.Empty;
            if (status_en.Equals("Transfer to ED"))
            {
            ...
            {
                bool IsAcceptNurse = (bool)opd.OPDHandOverCheckList?.IsAcceptNurse;
                bool IsAcceptPhysician = (bool)opd.OPDHandOverCheckList?.IsAcceptPhysician;
                var specId = Guid.Parse(opd.OPDHandOverCheckList.ReceivingUnitPhysicianId.ToString());
                var spec = unitOfWork.SpecialtyRepository.FirstOrDefault(x => x.Id == specId && !x.IsDeleted);
                if(!spec.IsPublish)
                {
                    IsAcceptNurse = false;
                ...
                {
                    if(IsAcceptNurse && IsAcceptPhysician)
                    {
                        IsAcceptNurse = false;
                        IsAcceptPhysician = false;
                    }
        ...
        {
            if (specialty.Id != hocl.HandOverUnitNurseId || receiving.Id != hocl.ReceivingUnitNurseId || reason != hocl.ReasonForTransfer)
            {
                hocl.HandOverUnitNurseId = specialty.Id;
                hocl.HandOverUnitPhysicianId = specialty.Id;
                hocl.ReceivingUnitNurseId = receiving.Id;
        ...
        {
            if (opd.IsTelehealth)
                return GetAllergyInInitialAssessmentForTelehealth(opd);
            return GetAllergyInInitialAssessmentForShortTerm(opd);
        }
        ...
        public dynamic PreAnesthe(OPD opd)
        {
        ...
        private class OPDVisitHistoryForOutpatientExaminationNote : VisitHistory
        {
            ...
            public override List<VisitModel> GetHistory()
            {
                ...
                //List<VisitModel> ipds = GetIPDHistory(customer.Id, this.visit.AdmittedDate); lấy ở EOC không lấy IPD
