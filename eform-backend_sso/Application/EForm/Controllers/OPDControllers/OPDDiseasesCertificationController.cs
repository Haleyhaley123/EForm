...
namespace EForm.Controllers.OPDControllers
{
    ...
    public class OPDDiseasesCertificationController : BaseOPDApiController
    {
        ...
        public IHttpActionResult GetDiseasesCertification(Guid visitId)
        {
            ...
            var datas = GetDatasFromOutpatientExaminationNoteData(outPatientExam);

            bool isExistDiseasesCertification = IsExistDiseasesCertification(datas, _is_ExistDiseasesCertification);
            if (!isExistDiseasesCertification)
                return Content(HttpStatusCode.NotFound, Message.OPD_OEN_NOT_FOUND);

            var hospital = visit.Site;
            var custumer = visit.Customer;
            var specialty = visit.Specialty;
            string userName_hospitalConfirmation = datas.FirstOrDefault(e => e.Code == code_hospitalConfirm)?.Value;
            User user_hospitalConfirmation = GetUserConfirm(userName_hospitalConfirmation);
            string userName_doctorConfirmed = datas.FirstOrDefault(e => e.Code == code_doctorConfirm)?.Value;
            ...
            return Content(HttpStatusCode.OK, new
            {
                ...
                PID = custumer?.PID,
                ExaminationDate = outPatientExam?.ExaminationTime?.ToString("dd/MM/yyyy"),
                Diagnose = FormatDiagnose(datas),
                TreatmentDirection = datas.FirstOrDefault(e => e.Code == "OPDOENTP0ANS") == null ? "" : datas.FirstOrDefault(e => e.Code == "OPDOENTP0ANS").Value,
