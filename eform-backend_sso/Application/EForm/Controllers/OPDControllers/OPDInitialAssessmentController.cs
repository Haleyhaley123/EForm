...
namespace EForm.Controllers.OPDControllers
{
    ...
    public class OPDInitialAssessmentController : BaseOPDApiController
    {
        ...
        public IHttpActionResult GetInitialAssessmentsForShortTermAPI(Guid id, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(id);
            if (opd == null)
            ...
            return Content(HttpStatusCode.OK, new
            {
                ...
                Username = opd.OPDInitialAssessmentForShortTerm?.UpdatedBy,
                opd.RecordCode,
                iafst.Id,
                iafst.Version,
                ClinicId = clinic?.Id,
        ...
        public IHttpActionResult UpdateInitialAssessmentsForShortTermAPI(Guid id, [FromBody] JObject request, string type)
        {
            var opd = GetOPD(id);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
            ...
            var user = GetUser();
            if (IsBlockAfter24h(opd.CreatedAt) && !HasUnlockPermission(opd.Id, "A02_007_080121_VE", user.Username))
            ...
            if (!IsSuperman() && !CanWriteShortTerm(user, iafst.UpdatedBy, iafst.Id, iafst.CreatedAt, iafst.UpdatedAt))
        ...
        public IHttpActionResult UpdateInitialAssessmentsForOnGoingAPI(Guid id, [FromBody] JObject request)
        {
            ...
            if (IsBlockAfter24h(opd.CreatedAt) && !HasUnlockPermission(opd.Id, "OPDIAFOGOP", user.Username))
                return Content(HttpStatusCode.BadRequest, Message.TIME_FORBIDDEN);
        ...
        public IHttpActionResult GetInfoFallRiskScreenings(Guid visitId, string type = "A02_007_220321_VE")
        {
        ...
        [Permission(Code = "OINAS7")]
        public IHttpActionResult GetFallRiskScreenings(Guid visitId, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(visitId);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
        ...
        [Permission(Code = "OINAS7")]
        public IHttpActionResult GetFallRiskScreeningForOnGoingAPI(Guid visitId, Guid id, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(visitId);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
        ...
        public IHttpActionResult UpdateFallRiskScreeningForOnGoingAPI(Guid visitId, [FromBody] JObject request, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(visitId);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
        ...
        [Permission(Code = "OINAS8")]
        public IHttpActionResult UpdateFallRiskScreeningForOnGoingAPI(Guid visitId, Guid id, [FromBody] JObject request, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(visitId);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
            ...
            var user = GetUser();
            if (IsBlockAfter24h(opd.CreatedAt) && !HasUnlockPermission(opd.Id, type, user.Username))
                return Content(HttpStatusCode.BadRequest, Message.TIME_FORBIDDEN);
        ...
        public IHttpActionResult SyncFallRiskScreeningForOnGoingAPI(Guid visitId, string type = "A02_007_220321_VE")
        {
            var opd = GetOPD(visitId);
            if (opd == null)
                return Content(HttpStatusCode.NotFound, Message.OPD_NOT_FOUND);
        ...
        public IHttpActionResult CreateInitialAssessmentsForTelehealthAPI(Guid id)
        {
            ...
            var user = GetUser();
            if (IsBlockAfter24h(opd.CreatedAt) && !HasUnlockPermission(opd.Id, "OPDIAFTP", user.Username))
                return Content(HttpStatusCode.BadRequest, Message.TIME_FORBIDDEN);
        ...
        public IHttpActionResult GetInitialAssessmentsForTelehealthAPI(Guid id, string type = "OPDIAFTP")
        {
            ...
            var user = GetUser();
            var iafth = opd.OPDInitialAssessmentForTelehealth;
            var IsLocked = Is24hLocked(opd.CreatedAt, opd.Id, type, user.Username);
        ...
        public IHttpActionResult DeleteInitialAssessmentsAPI(Guid id, [FromBody] DeleteMedicalRecord request)
        {
            ...
            var user = GetUser();
            if (IsBlockAfter24h(opd.CreatedAt) && !HasUnlockPermission(opd.Id, new string[] { "OPDIAFTP", "OPDIAFSTOP", "OPDIAFOGOP" }, user.Username))
                return Content(HttpStatusCode.BadRequest, Message.TIME_FORBIDDEN);
        ...
        [HttpPost]
        [CSRFCheck]
        [Route("api/OPD/InitialAssessments/ChoosePrimaryDoctor/{id}")]
        [Permission(Code = "OINAS16")]
        public IHttpActionResult ChoosePrimaryDoctorAPI(Guid id, [FromBody] JObject request)
        {
            ...
            var user = GetUser();
            if (IsBlockAfter24h(current_opd.CreatedAt) && !HasUnlockPermission(current_opd.Id, new string[] { "OPDIAFTP", "OPDIAFSTOP", "OPDIAFOGOP" }, user.Username))
                return Content(HttpStatusCode.BadRequest, Message.TIME_FORBIDDEN);
        ...
        private void HandleInitialAssessmentForShortTermDatas(OPD opd, OPDInitialAssessmentForShortTerm iafst, JToken request_iafst_data)
        {
            ...
            var visit_util = new VisitAllergy(opd, "OPD");
        ...
        #endregion
        private bool CanWriteShortTerm(User user, string updated_by, Guid idShortTerm, DateTime? created_at, DateTime? updated_at)
        {
        ...
        public OPDPreAnesthesiaHandOverCheckList GetAnesthesia(OPD opd)
        {
            if (opd.TransferFromId == null) return null;
            var check = unitOfWork.OPDPreAnesthesiaHandOverCheckListRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == opd.TransferFromId.Value);
            return check;
        }
