...
namespace EForm.Controllers.GeneralControllers
{
    ...
    public class FileController : BaseApiController
    {
        ...
        public IHttpActionResult DeleteFileFromForms(Guid FormId,[FromBody] Param param)
        {
            try
            {
                var list_files = unitOfWork.UploadImageRepository.Find(e => !e.IsDeleted && e.FormId == FormId).ToList();
                if (list_files != null && list_files.Count > 0)
                {
                ...
                if (!String.IsNullOrEmpty(param.Urls))
                {
                    ...
                    foreach (var url in urls)
                    {
                        ...
                        var sub_file = unitOfWork.UploadImageRepository.FirstOrDefault(e => e.Url == sub_url);
                        if (sub_file != null)
                        {
        ...
        public IHttpActionResult GetListWithPaging(string type, string formcode, Guid visitId, [FromUri] UploadImageModel param)
        {
            ...
            if (checkfrom == null) return Content(HttpStatusCode.NotFound, new { ViMessage = "Không tìm thấy formcode! ", EnName = "Formcode is not found!" });
            ...
            var list_groupb = fromForms.GroupBy(p => p.FormId, (key, g) => new { Id = key, Data = g.ToList() });
            foreach (var i in list_groupb)
            {
                UploadModel sub_item = new UploadModel();
                var b = i.Data;
                sub_item.Id = i.Id;
                sub_item.CreatedAt = b.FirstOrDefault()?.CreatedAt;
                sub_item.CreatedBy = b.FirstOrDefault()?.CreatedBy;
                sub_item.Title = b.FirstOrDefault()?.Title;
                sub_item.Content = b.FirstOrDefault()?.Name;
            ...
            if (!string.IsNullOrEmpty(param.Title))
                result_list = (from r in result_list where r.Title.ToUpper().Contains(param.Title.ToUpper()) select r).ToList();
        ...
        public IHttpActionResult DeleteForm(string type, string formcode, Guid visitId, Guid id)
        {
            ...
            //else if(fromForms.Count > 0)
            //{
            //    //foreach(var i in fromForms)
            //    //{
            //    //    i.IsDeleted = true;
            //    //    unitOfWork.UploadImageRepository.Update(i);
            //    //    unitOfWork.Commit();
            //    //}
            //    return Content(HttpStatusCode.BadRequest, new { ViMessage = "Bạn không có quyền xoá", EnMessage = "" });
            //}           
            return Content(HttpStatusCode.OK, new { ViMessage = "Xoá thành công", EnMessage = "" });
        }
        [HttpGet]
        ...
        public IHttpActionResult GetFormByVisitId(string type, string formcode, Guid visitId, Guid id)
        {
            ...
            var form2 = unitOfWork.UploadImageRepository.Find(e => !e.IsDeleted && e.FormId == id && !e.Title.Contains("UPFROMLOADFILE")).ToList();
            ...
            if (form != null)
            {
                ...
                if(title != null && !title.Contains("UPFROMLOADFILE"))
                {
                    ...
                    data = new
                    {
                        ...
                        Datas = GetFormData(visitId, id, "UPLOADFILE"),
        ...
        private List<FormDataValue> GetFormData(Guid visitId, Guid formId, string formCode)
        {
