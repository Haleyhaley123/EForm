...
namespace EForm.Controllers
{
    ...
    public class DoctorPlaceDiagnosticsOrderController : BaseApiController
    {
        ...
        public IHttpActionResult GetCustomer([FromUri] HISCustomerParameterModel request)
        {
            if (!request.Validate())
            ...
            var Visit = OHClient.GetOpenVisitCode(request.PID);
        ...
        public IHttpActionResult GetCustomerVisitInfo([FromUri] HISCustomerParameterModel request)
        {
            if (!request.Validate())
                return Content(HttpStatusCode.BadRequest, Message.FORMAT_INVALID);
            var current_username = getUsername();
            ...
            var Visit = OHClient.GetOpenVisitCode(PID);
        ...
        public IHttpActionResult LastItemChargeByDoctor([FromUri] ServicesParameterModel request = null)
        {
            ...
            var charge_id = (from charge_item_sql in unitOfWork.ChargeItemRepository.AsQueryable().Where(
                            e => e.CreatedBy == username && (e.ServiceType == request.Type)
                             ...
                             select charge_item_sql.ServiceId).Take(30).ToList();
        ...
        [HttpPost]
        [CSRFCheck]
        [Route("api/DoctorPlaceDiagnosticsOrder/Charge/")]
        [Permission(Code = "DRS0007")]
        public IHttpActionResult SubmitCharge([FromBody] ChargeModel request)
        {
            ...
            if (IsVisitOpen(request.Charge.PatientVisitId.Value))
            {
                using (DataAccess.Repository.IUnitOfWork _unitOfWork = new EfUnitOfWork())
                {
                    Customer customer = _unitOfWork.CustomerRepository.FirstOrDefault(c => !c.IsDeleted && c.PID == request.PID);
                    ...
                    _unitOfWork.ChargeRepository.Add(request.Charge);
                    _unitOfWork.Commit();
                    ...
                    foreach (var data in request.ChargeItems)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var ServiceDepartmentId = getServiceDepartment(data.PatientLocationId, data.ItemId);
                        ...
                        {
                            _unitOfWork.ChargeItemMicrobiologyRepository.Add(data.Microbiology);
                            _unitOfWork.Commit();
                        }
                        if (data.Pathology != null)
                        {
                            _unitOfWork.ChargeItemPathologyRepository.Add(data.Pathology);
                            _unitOfWork.Commit();
                        }
                        ind++;
                    }
                    ...
                    {
                        _unitOfWork.ChargeItemRepository.Add(item);
                    }
                    _unitOfWork.Commit();
                    _unitOfWork.Dispose();
                }
                ...
                {
                    rsRad = Oh_Service.PlaceRadOrder(listRadChargeItem.Select(i => i.Id).ToArray());
                }
                if (listAllChargeItem.Count > 0)
                {
                    rsAll = Oh_Service.PlaceAlliedOrder(listAllChargeItem.Select(i => i.Id).ToArray());
                }
                if (listLabChargeItem.Count > 0)
                {
                    rsLab = Oh_Service.PlaceLabOrder(listLabChargeItem.Select(i => i.Id).ToArray());
                }
                return Ok(new
                {
                    Total = request.ChargeItems.Count,
        ...
        [Permission(Code = "DRS0007")]
        public IHttpActionResult SubmitChargeV2([FromBody] ChargeModel request)
        {
            // return Content(HttpStatusCode.BadRequest, Message.PLACING_ORDER_VISIT_IS_CLOSED);
            if (IsVisitOpen(request.Charge.PatientVisitId.Value))
            {
        ...
        [Permission(Code = "DRS0007")]
        public IHttpActionResult SubmitChargeV2Drs(Guid ChargeId)
        {
            ...
            if (IsVisitOpen(charge.PatientVisitId.Value))
            {
                ...
                rsRad = Oh_Service.PlaceRadOrder(new List<Guid>().ToArray(), ChargeId);
                rsAll = Oh_Service.PlaceAlliedOrder(new List<Guid>().ToArray(), ChargeId);
                rsLab = Oh_Service.PlaceLabOrder(new List<Guid>().ToArray(), ChargeId);
        ...
        public IHttpActionResult CancelCharge([FromBody] ChargeModel request)
        {
            var Oh_Service = new OHService();
            ...
            if (isOpen || isAuth)
            {
                ...
                listChargeItem = UpdatedChargeFromOH(listChargeItem);
                List<ChargeItem> listLabChargeItem = new List<ChargeItem>();
                List<ChargeItem> listRadChargeItem = new List<ChargeItem>();
                List<ChargeItem> listAllChargeItem = new List<ChargeItem>();
                var rsLab = new OHServiceResult();
                ...
                if (listLabChargeItem.Count > 0)
                {
                    rsLab = Oh_Service.CancelLabOrder(listLabChargeItem);
                }
                if (listRadChargeItem.Count > 0)
                {
                    rsRad = Oh_Service.CancelRadOrder(listRadChargeItem);
                }
                if (listAllChargeItem.Count > 0)
                {
                    rsAll = Oh_Service.CancelAlliedOrder(listAllChargeItem);
                }
                unitOfWork.Commit();
                return Ok(new
                {
        ...
        public IHttpActionResult GetUpdateCharge([FromBody] ChargeModel request)
        {
            ...
            {
                listChargeItemUpdated = UpdatedChargeFromOH(listChargeItems);
            }
        ...
        private List<ChargeItem> UpdatedChargeFromOH(List<ChargeItem> listChargeItems)
        {
            var sum = listChargeItems.Count;
            if (sum > 0)
            {
                var position = 0;
                do
                {
                    ...
                    var listChargeI = listChargeItems.GetRange(position, count);
                    GetUpdateChargesFromOH(listChargeI);
                    sum = sum - count;
                    position += count;
                } while (sum > 0);
            }
        ...
        private List<ChargeItem> GetUpdateChargesFromOH(List<ChargeItem> listChargeItems)
        {
            if (listChargeItems.Count > 0)
            {
                ...
                var listUpdatedCharge = OHClient.getUpdateCharge(
                ...
                if (listUpdatedCharge.Count > 0)
                {
                    foreach (var u in listUpdatedCharge)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            {
                                if (u.PlacerOrderStatus == CpoePlacerOrderStatus.Verified && (item.Status == Constant.ChargeItemStatus.Placing || item.Status == Constant.ChargeItemStatus.DRSFailed))
                                {
        ...
        private bool isSetupLabDep(Guid HISId, ServicesGetPriceParameterModel request)
        {
            ...
            if (LabOrderableRefData != null)
            {
                var ServiceDepartment = getServiceDepartment(request.PatientLocationId, HISId);
                if (ServiceDepartment == null)
                {
                    hasLabDepartment = false;
                }
        ...
        {
            return unitOfWork.ChargeItemRepository.Find(item => !item.IsDeleted && ChargeId == item.ChargeId).ToList().OrderByDescending(e => e.CreatedAt).Select(e => foE(e)).ToList();
        }
        ...
        private Guid? getServiceDepartment(Guid patientLocationId, Guid HISId)
        {
            return OHClient.getServiceDepartmentId(patientLocationId, HISId);
        }
        ...
        private bool HasChargeItem(Customer customer)
        {
            var current_username = getUsername();
            return unitOfWork.ChargeVistRepository.Find(e => !e.IsDeleted && e.CustomerId == customer.Id && (current_username.ToLower() == e.DoctorAD.ToLower() || current_username.ToLower() == e.CreatedBy.ToLower())).Count() > 0;
        }
    }
}