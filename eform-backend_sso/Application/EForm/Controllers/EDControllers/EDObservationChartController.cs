        ...
        public IHttpActionResult GetObservationChart(Guid id)
        {
            ...
            return Ok(new
            {
                ...
                Datas = observation_chart.ObservationChartDatas.Where(e=> !e.IsDeleted).OrderBy(oc => oc.NoteAt).Select(oc => new { 
                    oc.Id, NoteAt = oc.NoteAt?.ToString(Constant.TIME_DATE_FORMAT_WITHOUT_SECOND), oc.Other, oc.SysBP, oc.DiaBP, oc.Pulse,
                    oc.Temperature, oc.Resp, oc.SpO2, oc.RestPainScore, oc.MovePainScore, Fullname = oc.CreatedBy 
                }).ToList(),
            });
        }
