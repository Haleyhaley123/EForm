...
namespace EForm.Controllers.PrescriptionControllers
{
    ...
    public class PrescriptionController : BaseApiController
    {
        ...
        public IHttpActionResult GetDetailPrescription(string prescriptionId, [FromUri] HISCustomerParameterModel request)
        {
            ...
            try
            {
                if (listPharmacies.Count > 0)
                {
                    ...
                    {
                        medicationInfo = GetMedicalInfo(visitCode, listPharmacies[0].PrescriberAD, listPharmacies[0].LocationCode, request.VisitId, true, pid);
                    }
                    else
                    {
                        medicationInfo = GetMedicalInfo(visitCode, listPharmacies[0].PrescriberAD, listPharmacies[0].LocationCode, request.VisitId, false, pid);
                    }
        ...
        private MedicationInfoModel GetMedicalInfo(string visitCode, string primaryDoctorUserName, string locationCode, Guid? visitId, bool isFromViHC, string PID)
        {
            MedicationInfoModel medicationInfo = new MedicationInfoModel();

            PrescriptionVisitModel visit = GetVisitByVisitCodeAndDoctor(visitCode, primaryDoctorUserName, locationCode, visitId);
                ...
                if (isFromViHC)
                {
                    ...
                    try
                    {
                        ...
                        if (response != null)
                        {
                            ...
                            if (root.Result != null && root.Result.VisitCode == visitCode && root.Result.DoctorAD.ToUpper() == primaryDoctorUserName.ToUpper())
                            {
                                // Lấy thông tin chiều cao
                                medicationInfo.CustomerMedicationInfo.Height = root.Result.Height;
        ...
        private PrescriptionVisitModel GetVisitByVisitCodeAndDoctor(string visitCode, string doctorUserName, string locationCode, Guid? visitId)
        {
