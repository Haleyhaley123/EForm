    ...
    public class BaseApiController : ApiController
    {
        ...
        protected dynamic GetInWaitingAcceptPatientByPID(string pid)
        {
            var in_transfer = unitOfWork.EDStatusRepository.Find(
                e => !e.IsDeleted &&
                !string.IsNullOrEmpty(e.EnName) &&
                (e.EnName.Contains("Transfer") || e.EnName.Equals("Admitted"))
            ).Select(e => e.Id).ToList();
        ...
        protected bool IPDIsBlock(IPD ipd, string formCode, Guid? formId = null)
        {
            // if (IsSuperman()) return false;
        ...
        protected void HandleUpdateOrCreateFormDatas(Guid VisitId, Guid FormId, string formCode, JToken request, dynamic visit = null, bool ischeck = false)
        {
            ...
            if (request != null)
            {
                ...
                if (visit != null)
                {
                    ...
                    {
                        var customer_util = new CustomerUtil(unitOfWork, visit.Customer);
                        customer_util.UpdateAllergy(allergy_dct);
                        var visit_util = new VisitAllergy(visit, visit_type);
        ...
        protected bool ICheckPermission(string username, string code)
        {
        ...
        protected List<MasterDataValue> GetMdValueInVisit(Guid visit_id, string[] md_coce)
        {
            return unitOfWork.FormDatasRepository.Find(e => md_coce.Contains(e.Code) && e.VisitId == visit_id)
                .Select(e => new MasterDataValue
                {
                    Code = e.Code,
                    Value = e.Value,
        ...
        protected CustomerModel getCustomerInfoEOC(EOC visit)
        {
            ...
            var data = unitOfWork.FormDatasRepository.Find(e => etrMasterDataCode.Contains(e.Code) && e.VisitId == visit.Id)
                .Select(e => new MasterDataValue
                {
                    Code = e.Code,
                    Value = e.Value,
        ...
        protected EOCInfo getEOCInfo(Guid visit_id, string type)
        {
            ...
            var to_visit = GetVisit((Guid)finded.ToVisitId);
            if (to_visit == null)
            {
                return new EOCInfo
                {
        ...
        protected MedicalRecordViewModel GetCareNote(Guid visit_id)
        {
            ...
            var formNewborn = unitOfWork.EIOConstraintNewbornAndPregnantWomanRepository.AsQueryable()
                             .Where(e => !e.IsDeleted && e.VisitId == visit_id && e.FormCode == "A02_062_050919_V")
        ...
        protected VisitAllergyModel getVisitAllergyModel(Guid VisitId, string visit_type)
        {
            if (visit_type == "ED")
            {
        ...
        protected VisitAllergyModel getEDAllergyModel(EDQueryModel edmodel, Guid visit_id)
        {
            //if (edmodel.CustomerIsAllergy != null) Không lấy dị ứng theo visit mà phải lấy theo sự thay đổi của đánh giá ban đầu 
            //{
            //    return new VisitAllergyModel()
            //    {
            ...
            //    };
            //}
            var visit = GetED(visit_id);
            var etr = visit.EmergencyTriageRecord;
            if (etr != null)
            {
                var data = etr.EmergencyTriageRecordDatas.Where(e => !e.IsDeleted && Constant.ED_ETR_ALLERGIC_CODE.Contains(e.Code))
                .Select(etrd => new { etrd.Code, etrd.Value }).ToList();
                var allergy_dct = new Dictionary<string, string>();
                ...
                return ConvertAllergy(allergy_dct);
            }
            return new VisitAllergyModel() { };
        }
        protected VisitAllergyModel getIPDAllergyModel(IPDQueryModel edmodel, Guid visit_id)
        {
            ...
            if (etr != null)
            {
                var data = etr.IPDInitialAssessmentForAdultDatas.Where(e => !e.IsDeleted && Constant.IPD_IAAU_ALLERGIC_CODE.Contains(e.Code))
                .Select(etrd => new { etrd.Code, etrd.Value }).ToList();
                var allergy_dct = new Dictionary<string, string>();
        ...
        protected VisitAllergyModel getEOCAllergyModel(OPDQueryModel edmodel, Guid visit_id)
        {
            ...
            //    };
            //}
            var etr = unitOfWork.EOCInitialAssessmentForShortTermRepository.Find(e => !e.IsDeleted && e.VisitId == visit_id).FirstOrDefault();
            if (etr != null)
            {
                var data = unitOfWork.FormDatasRepository.Find(e => !e.IsDeleted && e.FormId == etr.Id && Constant.OPD_IAFST_ALLERGIC_CODE.Contains(e.Code));
                var allergy_dct = new Dictionary<string, string>();
                foreach (var item in data)
                {
                    allergy_dct[Constant.CUSTOMER_ALLERGY_SWITCH[item.Code]] = item.Value;
        ...
        protected VisitAllergyModel getOPDAllergyModel(OPDQueryModel opdmodel, Guid visit_id)
        {
            //if (opdmodel.CustomerIsAllergy != null) Không lấy dị ứng theo visit mà phải lấy theo sự thay đổi của đánh giá ban đầu 
            //{
            //    return new VisitAllergyModel()
            //    {
            //        IsAllergy = opdmodel.CustomerIsAllergy,
            //        Allergy = opdmodel.CustomerAllergy,
            //        KindOfAllergy = opdmodel.CustomerKindOfAllergy
            //    };
            //}
            var visit = GetOPD(visit_id);
            var allergy_dct = new Dictionary<string, string>();
            var data = new List<FormDataValue>();
            if (visit.IsTelehealth)
            {
                var tlh = visit.OPDInitialAssessmentForTelehealth;
                if (tlh != null)
                {
                    data = tlh.OPDInitialAssessmentForTelehealthDatas.Where(e => !e.IsDeleted && Constant.OPD_IAFTP_ALLERGIC_CODE.Contains(e.Code))
            ...
            {
                var ert = visit.OPDInitialAssessmentForShortTerm;
                if (ert != null)
                {
                    data = ert.OPDInitialAssessmentForShortTermDatas.Where(e => !e.IsDeleted && Constant.OPD_IAFST_ALLERGIC_CODE.Contains(e.Code))
                        .Select(etrd => new FormDataValue() { Code = etrd.Code, Value = etrd.Value }).ToList();
                }
            }
            foreach (var item in data)
        ...
        {
            return unitOfWork.IPDSetupMedicalRecordRepository.FirstOrDefault(x => x.Formcode.ToUpper() == formCode.ToUpper() && x.SpecialityId == specialityId);
        }
        protected IPDMedicalRecordOfPatients GetMedicalRecordOfPatients(string formCode, Guid? visitId, Guid? formId)
        {
            ...
            return unitOfWork.IPDMedicalRecordOfPatientRepository.FirstOrDefault(x => x.FormCode.ToUpper() == formCode.ToUpper() && x.VisitId == visitId && x.FormId == formId && !x.IsDeleted);
        }
        protected List<DiagnosticReportingCharge> GetDiagnosticReportingByPid(string PID)
        {
            var query = (from dia_sql in unitOfWork.DiagnosticReportingRepository.AsQueryable().Where(
                            ...
                            select new DiagnosticReportingCharge
                            {
                                ...
                                //PickupAtDate = dia_sql.UpdatedAt.Value.Date,
                                PickupBy = dia_sql.CreatedBy,
                                Id = dia_sql.Id,
                                NguoiTraKQ = dia_sql.UpdatedBy
                            });
    
            query = query.Where(e => e.PID == PID).OrderBy(m => m.PickupAt);
            return query.ToList();
        }
        protected List<DiagnosticReportingCharge> GetDiagnosticReportingByPid(string PID, string VisitCode)
        {
            var query = (from dia_sql in unitOfWork.DiagnosticReportingRepository.AsQueryable().Where(
                            e => !e.IsDeleted && e.Status == 2
                            )
                         ...
                         on dia_sql.ChargeItemId equals charge_item_sql.Id
                         join cus_sql in unitOfWork.CustomerRepository.AsQueryable()
                         on charge_item_sql.PatientId equals cus_sql.PID
                         join charge_sql in unitOfWork.ChargeRepository.AsQueryable()
                         on charge_item_sql.ChargeId equals charge_sql.Id
                         join charge_visit_sql in unitOfWork.ChargeVistRepository.AsQueryable()
                         on charge_sql.ChargeVisitId equals charge_visit_sql.Id
            ...
            query = query.Where(e => e.PID == PID && e.VisitCode == VisitCode);
            return query.ToList().OrderBy(m => m.ExamCompleted).ToList();
        }
        protected dynamic GroupDiagnosticReportingByPid(string PID)
        {
            ...
            return DiagnosticReporting.GroupBy(e => e.PickupAt.Value.Date).Select(e => new
            {
                RawDate = e.Key,
        ...
        {
            return unitOfWork.DiagnosticReportingRepository.Find(e => !e.IsDeleted && e.ChargeItemId == charge_item_id).Any();
        }
        protected List<VisitModel> GetAllInfoCustomerInAreIPD(IPD ipd)
        {
            ...
            var ipds = (from visit in unitOfWork.IPDRepository.AsQueryable().Where(
                        ...
                        select new VisitModel
                        {
                            ...
                            Status = new { stt_sql.EnName, stt_sql.ViName },
                            Type = "IPD",
                            SpecialtyId = visit.SpecialtyId,
        ...
        protected void CreateOrUpdateFormForSetupOfAdmin(Guid? visitId, Guid? formId, string formCode)
        {
        ...
        protected List<VisitModel> getIPDVisitByCustomerId(Guid customer_id)
        {
            ...
            var results = (from visit in unitOfWork.IPDRepository.AsQueryable().Where(
                           ...
                           select new VisitModel
                           {
                               ...
                               CustomerKindOfAllergy = visit.KindOfAllergy,
                               CreatedBy = visit.CreatedBy,
                           }).Where(e => e.StatusCode != "IPDNOEX").ToList();
