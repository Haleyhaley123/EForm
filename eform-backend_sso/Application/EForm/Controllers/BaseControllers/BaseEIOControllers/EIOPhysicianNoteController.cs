...
namespace EForm.Controllers.BaseControllers.BaseEIOControllers
{
    public class EIOPhysicianNoteController : BaseApiController
    {
        protected dynamic GetListEIOPhysicianNote(dynamic visit, string visit_type, string from, string to, string createdBy, int? sort = null, Guid? formId = null)
        {
            ...
            var responsive = from e in result
                             join c in unitOfWork.EIOFormConfirmRepository.AsQueryable()
                             .Where(e => !e.IsDeleted)
                             on e.Form.Id equals c.FormId into temp
                             from res in temp.DefaultIfEmpty()
                             select new
                             {
                                 e.Form,
                                 e.FullNameUser,
                                 e.FormContraintNewborn,
                                 confirm = res
                             };

            // thêm 2 kiểu sắp xếp 
            if (sort == null || sort == 1)
                responsive = responsive.OrderByDescending(e => e.Form.NoteTime);
            else
                responsive = responsive.OrderBy(e => e.Form.NoteTime);

            return responsive.ToList().Select(e => new
            {
                e.Form.Id,
