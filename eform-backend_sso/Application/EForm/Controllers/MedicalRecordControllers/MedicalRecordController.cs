...
namespace EForm.Controllers.MedicalRecordControllers
{
    ...
    {
        private readonly string[] StatusCode = { "EDIH", "OPDIH", "IPDIH", "EOCIH", "OPDWR", "EDWR", "IPDWR", "EOCWR" };
        [HttpGet]
        ...
        [Permission(Code = "MMERE2")]
        public IHttpActionResult CustomerDetailAPI(string pid, [FromUri] CustomerDetailParameterModel request)
        {
            var customer = unitOfWork.CustomerRepository.FirstOrDefault(e => !e.IsDeleted && !string.IsNullOrEmpty(e.PID) && e.PID == pid);
            if (customer == null)
                return Content(HttpStatusCode.NotFound, Message.CUSTOMER_NOT_FOUND);
            ...
            {
                var visit = results.FirstOrDefault();
                visitId = (Guid)visit.Id;
                recordCode = visit.RecordCode;
                statuscode = unitOfWork.EDStatusRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == visit.StatusId).Code;
            }
            ...
            return Content(HttpStatusCode.OK, new
            {
                Customer = new
                {
                    ...
                    DischargeDate = StatusCode.Contains(statuscode) ? null : visitModel?.DischargeDate,
        ...
        private VisitAllergyModel GetAllergyByVisitModel(VisitModel visitMode)
        {
            ...
            switch (visitMode.Type)
            {
                case "IPD":
                    return getIPDAllergyModel(new IPDQueryModel() { CustomerIsAllergy = null }, (Guid)visitMode.Id);
                case "OPD":
                    return getOPDAllergyModel(new OPDQueryModel() { CustomerIsAllergy = null }, (Guid)visitMode.Id);
                case "ED":
                    return getEDAllergyModel(new EDQueryModel() { CustomerIsAllergy = null }, (Guid)visitMode.Id);
                case "EOC":
                    return getEOCAllergyModel(new OPDQueryModel() { CustomerIsAllergy = null }, (Guid)visitMode.Id);
                default:
                    return new VisitAllergyModel() { };
            }
        }
        ...
        public IHttpActionResult MedicalRecordV2([FromBody] JObject request)
        {
            ...
            VisitGroupInfoModel visitGroup = listResult.FirstOrDefault(x => x.Id == Id);
