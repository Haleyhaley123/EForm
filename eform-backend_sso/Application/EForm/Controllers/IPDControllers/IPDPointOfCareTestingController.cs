...
namespace EForm.Controllers.IPDControllers
{
    ...
    public class IPDPointOfCareTestingController : BaseIPDApiController
    {
        ...
        private dynamic BuildIPDArterialBloodGasTestResponse(IPD visit, EDArterialBloodGasTest abgt)
        {
            ...
            if(visit.Version >= 7)
                if(IsCheckConfirm(abgt.Id))
                {
                    abgt.ExecutionTime = null;
                    abgt.ExecutionUserId = null;
                    unitOfWork.Commit();
                }
        ...
        private void UpdateIPDArterialBloodGasTest(EDArterialBloodGasTest abgt, JObject request, int version)
        {
            ...
            abgt.Upload = BuildJsonUploadFile(request["Upload"], "ArterialBloodGasTest");
        ...
        private dynamic BuildChemicalBiologyTestResponse(EDChemicalBiologyTest cbt)
        {
            ...
            if (ipd.Version >= 7)
                if (IsCheckConfirm(cbt.Id))
                {
                    cbt.ExecutionTime = null;
                    cbt.ExecutionUserId = null;
                    unitOfWork.Commit();
                }
        ...
        {
            cbt.Upload = BuildJsonUploadFile(request["Upload"], "ChemicalBiologyTest"); ;
