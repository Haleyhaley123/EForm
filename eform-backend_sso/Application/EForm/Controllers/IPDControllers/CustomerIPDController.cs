...
namespace EForm.Controllers.IPDControllers
{
    ...
    public class CustomerIPDController : BaseIPDApiController
    {
        ...
        public IHttpActionResult DetailCustomerAPI(Guid id, [FromUri]CustomerRequetDetailModel request)
        {
            ...
            var listMedicalRecords = GetMedicalRecords(id);
            ...
            return Content(HttpStatusCode.OK, new
            {
                ...
                Allergy = GetAllergyInfo(ipd),
                Allergy2 = GetAllergyByVisit(ipd),
                ...
                MedicalRecordType = listMedicalRecords,               
                customer.Height,
                customer.Weight,
                Danhgiabandau = CheckPromissoryNote(ipd.Id),
                ipd.IsDraft,
        ...
        private List<dynamic> CheckPromissoryNote(Guid visitId)
        {
            IPD ipd = GetIPD(visitId);
            var specialty = ipd.SpecialtyId;
            var checkSetup = (from set in unitOfWork.IPDSetupMedicalRecordRepository.AsQueryable()
                              where set.SpecialityId == specialty && set.IsDeploy && set.FormType == "PromissoryNote"
                              select new
                              {
                                  set.Type,
                                  Status = true
            ...
            var checkOfPatient = (from check in unitOfWork.IPDMedicalRecordOfPatientRepository.AsQueryable()
                                  join set in unitOfWork.IPDSetupMedicalRecordRepository.AsQueryable()
                                  on check.FormCode equals set.Formcode
                                  where set.FormType == "PromissoryNote" && !set.IsDeploy && set.SpecialityId == specialty && !check.IsDeleted && check.VisitId == visitId
                                  select new
                                  {
                                      set.Type,
                                      Status = true
            ...
            if (checkSetup == null && checkOfPatient == null)
                return null;
            List<dynamic> promissoryNotes = new List<dynamic>();
            if (checkSetup != null) promissoryNotes.AddRange(checkSetup);
            if (checkOfPatient != null) promissoryNotes.AddRange(checkOfPatient);
        ...
        private VisitAllergyModel GetAllergyByVisit(IPD visitMode)
        {
            if (visitMode == null)
                return new VisitAllergyModel() { };

            //if (visitMode.IsAllergy != null) /// Không lấy dị ứng theo visit mà phải lấy theo sự thay đổi của đánh giá ban đầu 
            //{
            //    return new VisitAllergyModel()
            //    {
            //        IsAllergy = visitMode.IsAllergy,
            //        Allergy = visitMode.Allergy,
            //        KindOfAllergy = visitMode.KindOfAllergy
            //    };
            //}

            return getIPDAllergyModel(new IPDQueryModel() { CustomerIsAllergy = null }, visitMode.Id);
        }
    }
}
