    ...
    public class PathologyMicrobiologyFormController : BaseApiController
    {
        ...
        public IHttpActionResult GetListPathologyMicrobiology([FromUri] DiagnosticReportingParameterModel param, string PID)
        {
            ...
            var query = (from kq in temp
                     ...
                     {
                         Id = kq.charge_item_sql.Id,
                         VisitCode = kq.charge_item_sql.VisitCode,
                         ServiceEnName = kq.ser_sql.ViName,
                         ServiceViName = kq.ser_sql.ViName,
                         CombinedName = kq.ser_sql.CombinedName,
                         ServiceCode = kq.charge_item_sql.ServiceCode,
                         ChargeId = kq.charge_item_sql.ChargeId,
                         ItemType = kq.charge_item_sql.ItemType,
                         CreatedAt = kq.charge_item_sql.CreatedAt,
                         UpdatedAt = kq.charge_item_sql.UpdatedAt,
                         ChargeItemMicrobiologyId = kq.charge_item_sql.ChargeItemMicrobiologyId,
                         ChargeItemPathologyId = kq.charge_item_sql.ChargeItemPathologyId,
                         SpecimenStatus = kq.charge_item_sql.SpecimenStatus
                     });
            ...
            var items = query.OrderByDescending(m => m.CreatedAt).Skip((param.PageNumber - 1) * param.PageSize)
                   .Take(param.PageSize)
                   .ToList();
        ...
        public IHttpActionResult Create(int type, Guid id)
        {
            ChargeItem chargeitem = GetChargeItem(id);
        ...
        //[Permission(Code = "DRS0007")]
        public IHttpActionResult Detail(string type, Guid id)
        {
            ChargeItem chargeItem = GetChargeItem(id);
            if (chargeItem == null) return Content(HttpStatusCode.BadRequest, Message.FORM_NOT_FOUND);
        ...
        private ChargeItem GetChargeItem(Guid id)
        {
            var chargeItem = unitOfWork.ChargeItemRepository.Find(e => !e.IsDeleted && e.Id == id).FirstOrDefault();
            if (chargeItem == null) return null;
            var listUpdatedCharge = OHClient.getUpdateChargeByChargeDetailId(chargeItem.ChargeDetailId.ToString());
