...
namespace EForm.Client
{
    public class OHClient : HISClient
    {
        ...
        public static List<LISInfor> getLISResultByPID(string pId)
        {
            string url_postfix = "";
            if (!string.IsNullOrEmpty(pId))
                url_postfix = string.Format("/EMRVinmecCom/1.0.0/getLISResultByPID?PID={0}", pId);
            var dataFromOH = RequestAPI(url_postfix, "Entries", "Entry");
        ...
        public static List<OpenVisitResult> GetOpenVisitCode(string pid)
        {
        ...
        public static List<HISChargeDetailModel> getUpdateCharge(string ChargeIds, string LabChargeIds, string RadChargeIds, string visit_code, string visit_type)
        {
            ...
            var response = RequestAsyncAPIAsync(url_postfix, "Entries", "Entry", 20);
            if (response != null)
        ...
        public static List<HISChargeDetailModel> getUpdateChargeByChargeDetailId(string ChargeIds)
        {
            string url_postfix = string.Format(
                "/EMRVinmecCom/1.0.0/getChargeDetailIdStatus?ChargeIds={0}",
                ChargeIds
            );
            var response = RequestAsyncAPIAsync(url_postfix, "Entries", "Entry", 20);
            if (response != null)
        ...
        internal static Guid? getServiceDepartmentId(Guid patientLocationId, Guid HISId)
        {
            ...
            var response = RequestAsyncAPIAsync(url_postfix, "Entries", "Entry");
            if (response != null)
            {
        ...
        private static List<OpenVisitResult> BuildOpenVisitResult(JToken visit_data)
        {
            ...
            foreach (JToken data in visit_data)
            {
                ...
                if (string.IsNullOrEmpty(ClosureDate) || ClosureDate == "null")
                {
                    ...
                    {
                        PatientLocationId = StringToGuid(data.Value<string>("PatientLocationId")),
                        PatientVisitId = StringToGuid(data.Value<string>("PatientVisitId")),
                        VisitCode = visit_code,
                        VisitTypeName = visit_type,
                        ActualVisitDate = FormatDateVisitCode(ActualVisitDate),
                        ...
                        CostCentreId = StringToGuid(data.Value<string>("CostCentreId"))
                    });
                }
            }
            return result.ToList();
        ...
        private static Guid? StringToGuid(string a)
        {
