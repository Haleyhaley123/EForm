...
namespace EForm.Client
{
    public class HISClient
    {
        ...
        {
            return RequestAsyncAPIAsync(url_postfix, json_collection, json_item, timeout);
        }

        public static JToken RequestAsyncAPIAsync(string url_postfix, string json_collection, string json_item, int? timeout = null)
        {
        ...
        public static string RequestAsyncAPIAsyncGetRawstring(string url_postfix, string json_collection, string json_item, int? timeout = null)
        {
            using (HttpClient client = new HttpClient())
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", ConfigurationManager.AppSettings["HIS_API_SERVER_TOKEN"]);
        ...
        protected static JToken FormatHttpContent(string raw_data, string json_collection, string json_item)
        {
            JObject json_data = JObject.Parse(raw_data);
            try
            {
                JToken data = json_data[json_collection][json_item];
                return data;
            }
            catch
            {
        ...
        protected static JToken FakeRequestAPI(string raw_data, string json_collection, string json_item)
        {
            JObject json_data = JObject.Parse(raw_data);
            try
            {
        ...
        public static List<dynamic> FakeBuildShortResultV2(Customer customer)
        {
            List<dynamic> result = new List<dynamic>();
            result.Add(new
            {
        ...
        private static SystemNotification GetNotification(string scope)
        {
            return unitOfWork.SystemNotificationRepository.FirstOrDefault(
                e => !e.IsDeleted &&
                !string.IsNullOrEmpty(e.Service) &&
        ...
        //    try
        //    {
        //        return ConfigurationManager.AppSettings["DEV_ACCOUNT"].Contains(getUsername());
        //    }
        //    catch (Exception)
        //    {
        //        return false;
