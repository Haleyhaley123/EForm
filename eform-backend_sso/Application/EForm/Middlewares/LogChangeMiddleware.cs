...
namespace EForm.Middlewares
{
    public class LogChangeMiddleware: OwinMiddleware
    {
        ...
        public async override Task Invoke(IOwinContext context)
        {
            ...
            if (!request_path.Contains("api") || InIgnoreWriteLog(request_path)/* || request_path.Contains("SessionChanged") */)
            {
            ...
            if (context.Request.Body.CanRead)
            {
                using (StreamReader reader = new StreamReader(context.Request.Body))
                {
                    ...
                    if (InSensitive(request_path))
                    {
                        Regex regex = new Regex("password\":\".*}");
                        try
                        {
                            log_request = regex.Replace(log_request, "*****\"}");
                        }
                        catch
                        {
                            log_request = "*****";
