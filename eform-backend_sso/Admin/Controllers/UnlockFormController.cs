            ...
            try
            {
                ...
                foreach (var form in listForm)
                {
                    foreach (var formId in listFormId)
                    {
                        ...
                        {
                            if (!CheckIsIdFormMatchForm(visit_id, Guid.Parse(formId), form["Code"]) && !CheckIsIdFormMatchToUnlockConfirm(visit_id, Guid.Parse(formId), form["Code"]))
                            {
                                data = new
                                {
                                    IsUnlock = false,
        ...
        private bool CheckIsIdFormMatchForm(Guid visitId, Guid formId, string formCode)
        {
            ...
            switch (formCode)
            {
                ...
                case "A03_029_050919_VE":
                    var formResults = unitOfWork.OrderRepository.AsQueryable().Where(
                    e => !e.IsDeleted &&
                    e.VisitId == visitId).ToList();
                    if (formResults != null && formResults.Any())
                    {
                        foreach (var formResult in formResults)
                        {
