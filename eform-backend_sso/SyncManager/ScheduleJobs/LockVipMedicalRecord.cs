...
namespace SyncManager.ScheduleJobs
{
    public class LockVipMedicalRecord : IJob
    {
        ...
        private void DoJob()
        {
            CustomLog.intervaljoblog.Info($"<LockVipMedicalRecordJob> Start!");
            try
            {
                ...
                if (ConfigurationManager.AppSettings["CF_LockVipPatientService_CS"] != null)
                {
                    ...
                    CustomLog.intervaljoblog.Info($"<LockVipMedicalRecordJob> Start with {last_update_time.ToString()}");
                }
                ...
                CustomLog.intervaljoblog.Info($"<LockVipMedicalRecordJob> Success with ED {ed_count} + OPD {opd_count} + EDC {eoc_count} + IPD {ipd_count}");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<LockVipMedicalRecordJob> Error: {0}", ex));
            }
        }
