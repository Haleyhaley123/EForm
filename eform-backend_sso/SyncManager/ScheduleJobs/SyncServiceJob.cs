...
namespace SyncManager.ScheduleJobs
{
    public class SyncServiceJob
    {
        ...
        protected Service GetOrCreateService(HISServiceModel item, ServiceGroup group)
        {
            try
            {
                ...
                if(group == null)
                {
                    CustomLog.intervaljoblog.Info(string.Format("<Sync OH Service> Group not found: {0}", item.ServiceGroupId.ToString()));
                }
        ...
        protected string DetectItemType(HISServiceModel item)
        {
            ...
            {
                return Constant.ChargeItemType.Rad;
            } else
            {
                ...
                {
                    return Constant.ChargeItemType.Lab;
                }
                else
                {
    ...
    public class SyncOHServiceJob : SyncServiceJob, IJob
    {
        ...
        private void DoJob()
        {
            ...
            //last_updated_service_group = new DateTime(2012, 01, 01);
            //last_updated_service = new DateTime(2012, 01, 01);
