...
namespace SyncManager.ScheduleJobs
{
    public class SyncRadiologyProcedureJob
    {
        ...
        protected RadiologyProcedurePlanRef GetOrCreateRadiologyProcedurePlanRef(RadiologyProcedurePlanRef item, out Service service)
        {
            ...
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH RadiologyProcedurePlanRefRepository> Error: {0}, {1}", ex, item.ShortCode));
                service = null;
                return null;
            }
        }
        protected Service updateServiceType(RadiologyProcedurePlanRef objRadiologyProcedure)
        {
            ...
            if (findItem != null)
            {
                ...
                {
                    findItem.ServiceType = Constant.ChargeItemType.Rad;
                }
                else
                {
    ...
    public class SyncOHRadiologyProcedureJob : SyncRadiologyProcedureJob, IJob
    {
        ...
        private void SyncRadiologyProcedureJob()
        {
            ...
            CustomLog.intervaljoblog.Info($"<Sync OH Radiology Procedure> Start!");
            try
            {
                var results = OHClient.GetRadiologyProcedure(last_updated);
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH Radiology Procedure> Total item: {0}", results?.Count()));
                int countItem = 0;
                ...
                foreach (RadiologyProcedurePlanRef item in results)
                {
                    ...
                    {
                        countItem++;
                        CustomLog.intervaljoblog.Info(string.Format("<Sync OH radiology Procedure> Info {0}: Name: {1}]", countItem, item.RadiologyProcedureNameL));
                    }
                ...
                unitOfWork.Commit();
                CustomLog.intervaljoblog.Info($"<Sync Radiology Procedure> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync Radiology Procedure> Error: {0}", ex));
            }
        }
