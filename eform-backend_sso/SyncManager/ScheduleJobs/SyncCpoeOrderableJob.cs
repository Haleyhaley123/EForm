...
namespace SyncManager.ScheduleJobs
{
    public class SyncCpoeOrderableJob
    {
        ...
        protected CpoeOrderable GetOrCreateCpoeOrderable(CpoeOrderable item, out Service service)
        {
            ...
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH CpoeOrderable> Error: {0}", ex));
                service = null;
                return null;
            }
        }
        ...
        protected LabOrderableRef GetLabOrderableRef(LabOrderableRef item, out Service service)
        {
            ...
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH LabOrderableRef> Error: {0}", ex));
                service = null;
                return null;
            }
        }
        ...
        protected Service updateServiceType(LabOrderableRef labOrderableRef)
        {
            ...
            if(findItem != null)
            {
                ...
                {
                    findItem.ServiceType = Constant.ChargeItemType.Lab;
                }
                else
                {
                    findItem.ServiceType = Constant.ChargeItemType.Allies;
                }
    ...
    public class SyncOHCpoeOrderableJob : SyncCpoeOrderableJob, IJob
    {
        ...
        private void SyncCpoeOrderableJob()
        {
            ...
            CustomLog.intervaljoblog.Info($"<Sync OH Cpoe Orderable> Start!");
            try
            {
                ...
                foreach (CpoeOrderable item in results)
                {
                    ...
                    {
                        countItem++;
                        CustomLog.intervaljoblog.Info(string.Format("<Sync OH Cpoe Orderable> Info {0}: Name: {1}]", countItem, item.FillerNameL));
                    }
                ...
                unitOfWork.Commit();
                CustomLog.intervaljoblog.Info($"<Sync OH Orderable> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH Orderable> Error: {0}", ex));
            }
            unitOfWork.Dispose();
        }
        ...
        private void SyncLabOrderableRefJob()
        {
            ...
            CustomLog.intervaljoblog.Info($"<Sync OH Lab Orderable Ref> Start!");
            try
            {
                var results = OHClient.GetLabOrderableRef(last_updated);
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH labOrderableRef> Total item: {0}", results?.Count()));
                int countItem = 0;
                List<Guid> listServiceGroupIds = new List<Guid>();
                foreach (LabOrderableRef item in results)
                {
                    ...
                    {
                        countItem++;
                        CustomLog.intervaljoblog.Info(string.Format("<Sync OH Lab Orderable Ref> Info {0}: Name: {1}]", countItem, item.NameL));
                    }
                    if(service != null && !listServiceGroupIds.Contains(service.ServiceGroupId.Value))
                    {
                        listServiceGroupIds.Add(service.ServiceGroupId.Value);
                ...
                UpdateServiceGroupType(listServiceGroupIds);
                unitOfWork.Commit();
                CustomLog.intervaljoblog.Info($"<Sync OH Lab Orderable Ref> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<Sync OH Lab Orderable Ref> Error: {0}", ex));
            }
        }
