...
namespace SyncManager.ScheduleJobs
{
    public class ClearNullData : IJob
    {
        ...
        private void DoJob()
        {
            CustomLog.intervaljoblog.Info($"<ClearNullData> Start!");
            try
            {
                ...
                CustomLog.intervaljoblog.Info($"<ClearNullData> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<ClearNullData> Error: {0}", ex));
            }
        }
    }
}