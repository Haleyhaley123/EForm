...
namespace SyncManager.ScheduleJobs
{
    public class SendSMSUtilDischargedJob : IJob
    {
        ...
        private void DoJob()
        {
            CustomLog.intervaljoblog.Info($"<SendSMSUtilDischargedJob> Start!");
            ...
            {
                CustomLog.intervaljoblog.Info($"<SendSMSUtilDischargedJob> END with status config is null");
                return;
            }
            ...
            foreach (var opd in opds)
            {
                ...
                if (mailSend == null || (mailSend.Status != "SENDED" && mailSend.Status != "SENDING" && mailSend.ErrorCount < 3))
                {
            ...
            unitOfWork.Dispose();
            CustomLog.intervaljoblog.Info($"<SendSMSUtilDischargedJob> END!");
        }
