    ...
    public class SyncDataHC : IJob
    {
        ...
        public void DoJob()
        {
            try
            {
                ...
                var base_start_time = DateTime.ParseExact("17:30 13/10/2022", Constant.TIME_DATE_FORMAT_WITHOUT_SECOND, null);
                ...
                if (!DrsHCSyncIsOn)
                {
                    CustomLog.intervaljoblog.Info($"<SyncDataHC> END with DrsHCSyncIsOff");
                    return;
                }
                ...
                var resultFromDataHc = OHClient.SyncDataHc(from.ToString(Constant.DATETIME_SQL), to.ToString(Constant.DATETIME_SQL));
                CustomLog.intervaljoblog.Info($"<GET DataHC> " + resultFromDataHc.Count().ToString());
                foreach (var item in resultFromDataHc)
                {
                    var service = unitOfWork.ServiceRepository.FirstOrDefault(e => !e.IsDeleted && e.IsDiagnosticReporting && e.Code == item.ServiceCode);
                ...
                CustomLog.intervaljoblog.Info($"<Sync DataHC> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Error(string.Format("<Sync DataHC Error: {0}, Job is stoped", ex.ToString()));
                unitOfWork.Dispose();
            }
            unitOfWork.Dispose();
        }
        ...
        private void CreateOrUpdateChargeItem(DataHCModel data, Guid CustomerId, Guid chargeId, Service service)
        {
            ...
            //                ChargeItemType = Constant.ChargeItemType.Allies,
            //                ItemType  = 2
            //            };
