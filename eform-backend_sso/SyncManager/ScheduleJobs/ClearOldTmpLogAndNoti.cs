...
namespace SyncManager.ScheduleJobs
{
    public class ClearOldTmpLogAndNoti : IJob
    {
        ...
        private void DoJob()
        {
            CustomLog.intervaljoblog.Info($"<ClearOldTmpLogAndNoti> Start!");
            try
            {
                ...
                //);
                unitOfWork.Commit();
                CustomLog.intervaljoblog.Info($"<ClearOldTmpLogAndNoti> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<ClearOldTmpLogAndNoti> Error: {0}", ex));
            }
        }
    }
}