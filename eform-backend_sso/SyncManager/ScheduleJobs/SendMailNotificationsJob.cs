...
namespace SyncManager.ScheduleJobs
{
    public class SendMailNotificationsJob : IJob
    {
        ...
        private async void DoJob()
        {
            ...
            CustomLog.intervaljoblog.Info($"<SendMailNotificationsJob> Start!");
            ...
            try
            {
                ...
                CustomLog.intervaljoblog.Info($"<SendMailNotificationsJob> Success!");
            }
            catch (Exception ex)
            {
                CustomLog.intervaljoblog.Info(string.Format("<SendMailNotificationsJob> Error: {0}", ex));
            }
            unitOfWork.Dispose();
        }
