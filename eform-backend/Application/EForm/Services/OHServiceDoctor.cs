...
namespace EForm.Services
{
    public class OHServiceDoctor
    {
        ...
        public Guid? _ChargeId { get; set; }
        public Guid[] ChargeItemId { get; set; } = new List<Guid>().ToArray();
        public IEnumerable<ChargeItem> _ChargeItems { get; set; }
        public IEnumerable<ChargeItem> ChargeItems { 
            get
            {
                if (_ChargeId == null)
                {
                    _ChargeItems = ExecQuery.getChargeItemByArrayId(this.ChargeItemId);
                }
                return _ChargeItems;
            } 
        }
       
        public OHServiceDoctor(Guid? chargeId)
        {
            _ChargeId = chargeId;
            if (_ChargeId != null)
            {
                _ChargeItems = ExecQuery.getChargeItemById((Guid)this._ChargeId);
            }
        }
        public OHServiceDoctor()
        {

        } 

        public async Task<OHServiceResult> PlaceLabOrderAsync()
        {
            var ok = 0;
            var failed = 0;
            var listChargeItem = ChargeItems.Where(c =>c.ChargeItemType == Constant.ChargeItemType.Lab).ToList();
                ...
                do
                {
                    ...
                    if (orders != string.Empty)
                    {
                        ...
                        if (res.Key == HttpStatusCode.OK)
                        {
                            ...
                            List<ChargeItem> listUpdate = new List<ChargeItem>();
                            foreach (XElement element in xmlResponse.Descendants(b + "Order"))
                            {
                                ...
                                if (findCharge != null)
                                {
                                    ...
                                    listUpdate.Add(findCharge);
                                    // unitOfWork.ChargeItemRepository.Update(findCharge);
                                }
                            }
                            ExecQuery.updateChargeItem(Mapper.Map<List<ChargeItem>, List<ChargeItemDto>>(listUpdate));
                            // unitOfWork.Commit();
        ...
        public async Task<OHServiceResult> PlaceAlliedOrderAsync()
        {
            var ok = 0;
            var failed = 0;
            var listChargeItem = ChargeItems.Where(c => c.ChargeItemType == Constant.ChargeItemType.Allies).ToList();
            var sum = listChargeItem.Count;
            var total = listChargeItem.Count;
            if (sum > 0)
            {
                ...
                var customer = ExecQuery.getCustomerByPId(firstChargeItem.PatientId);
                do
                {
                    var count = sum > MaximumNumberOfItemPerRequest ? MaximumNumberOfItemPerRequest : sum;
                    var listChargeI = listChargeItem.GetRange(position, count);
        ...
        public async Task<OHServiceResult> PlaceRadOrderAsync()
        {
            var ok = 0;
            var failed = 0;
            var listChargeItem = ChargeItems.Where(c => c.ChargeItemType == Constant.ChargeItemType.Rad).ToList();
            var sum = listChargeItem.Count;
            var total = listChargeItem.Count;
            if (sum > 0)
            {
                ...
                do
                {
                    ...
                    docTemplate = docTemplate.Replace("{State}", Constant.AlliesRadRequestStatus.New);
                    string orders = string.Empty;
                    List<ChargeItem> listUpdateI = new List<ChargeItem>();
                    foreach (var c in listChargeI)
                    {
                    ...
                    ExecQuery.updateChargeItem(Mapper.Map<List<ChargeItem>, List<ChargeItemDto>>(listUpdateI));
                    // unitOfWork.Commit();
