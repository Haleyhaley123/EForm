...
namespace EForm.Controllers.PrescriptionControllers
{
    ...
    public class PrescriptionController : BaseApiController
    {
        ...
        private PrescriptionVisitModel GetVisitByVisitCodeAndDoctor(string visitCode, string doctorUserName, string locationCode, Guid? visitId, Guid? customerId)
        {
            ...
            // Tạo format mới cho visitCode
            // Tránh trường hợp kiểu abcd contain ab -> true
            string visitCodeNewFormat = string.Format(",{0},", visitCode);
            ...
            if (visitId == null)
            {
                ...
                .Find(e => !e.IsDeleted && ("," + e.VisitCode + ",").Contains(visitCodeNewFormat) && e.PrimaryDoctorId != null && e.CustomerId == customerId)
                ...
                listOpdVisits = unitOfWork.OPDRepository
                    .Find(e => !e.IsDeleted && ("," + e.VisitCode + ",").Contains(visitCodeNewFormat) && e.PrimaryDoctorId != null && e.PrimaryDoctor.Username.ToUpper() == doctorUserName.ToUpper() && e.CustomerId == customerId)
                    .Select(e => new PrescriptionVisitModel
                    {
                        VisitType = "OPD",
                        VisitId = e.Id,
