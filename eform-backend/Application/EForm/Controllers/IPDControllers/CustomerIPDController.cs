...
namespace EForm.Controllers.IPDControllers
{
    ...
    {
        // private InHospital in_hospital = new InHospital(unitOfWork);
        ...
        public IHttpActionResult CreateCustomerAPI([FromBody] CustomerEDParameterModel request)
        {
            ...
            InHospital in_hospital = new InHospital(unitOfWork);
            var in_hospital_status_id = in_hospital.GetStatus("IPD").Id;
        ...
        public IHttpActionResult DetailCustomerAPI(Guid id, [FromUri] CustomerRequetDetailModel request)
        {
            ...
            var md_setup_menu = GetMasterDatasSetUpForm();
            var forms_setup = GetSpecialtySetupForm((Guid)ipd.SpecialtyId, md_setup_menu);
            var list_form_final = GetMedicalRecordsSavedOrSetup(id, md_setup: md_setup_menu, specialty_setupForm: forms_setup);
            ...
            return Content(HttpStatusCode.OK, new
            {
                ...
                customer.Height,
                customer.Weight,
                Danhgiabandau = CheckPromissoryNote(ipd.Id, forms_setup, md_setup_menu),
                ipd.IsDraft,
        ...
        public IHttpActionResult SyncCustomerInfo(Guid id, string pid)
        {
            ...
            in_hospital.SetState(cus.Id, id, null, null);
            var in_hospital_visit = in_hospital.GetVisit();
            if (in_hospital_visit != null)
                return Content(HttpStatusCode.BadRequest, in_hospital.BuildErrorMessage(in_hospital_visit));
        ...
        private object[] CheckPromissoryNote(Guid visitId, List<MasterData> list_setup, List<MasterData> md_setup_menu)
        {
            List<MasterData> forms_saved = GetFormPatienSaved(visitId, md_setup_menu);
