...
namespace EForm.Controllers.GeneralControllers
{
    public class AccountController : BaseApiController
    {
        ...
        public HttpResponseMessage GetLogout()
        {
            ...
            resp.Content = new ObjectContent<dynamic>(Message.SUCCESS, new JsonMediaTypeFormatter());
            return resp;
        }

        //api cho tester check Permission
        [HttpGet]
        [Route("api/Tetser_CheckPermission/{code}")]
        public IHttpActionResult CheckPermission(string code)
        {
            var permission = unitOfWork.ActionRepository.FirstOrDefault(e => !e.IsDeleted && e.Code == code);
            if (permission == null) return Content(HttpStatusCode.NotFound, "Quyền không tồn tại");
            return Content(HttpStatusCode.OK, permission.Name);
        }
