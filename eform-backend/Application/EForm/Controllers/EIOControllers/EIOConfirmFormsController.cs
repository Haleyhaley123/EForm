...
using EForm.Utils;
using Helper.Files;
using EForm.Controllers.ESignControllers;
using EForm.Controllers.BaseControllers;
using DataAccess.Models.EsignModel;
using Bussiness.ESign;

namespace EForm.Controllers.EIOControllers
{
    [SessionAuthorize]
    public class EIOConfirmFormsController : BaseEDApiController
    {
        [HttpGet]
        ...
        public IHttpActionResult CreatedConfirmByFormId(Guid visitId, Guid formId, string formCode, [FromBody] JObject request)
        {
            ...
            if (string.IsNullOrWhiteSpace(req_userName) || string.IsNullOrWhiteSpace(req_userName)) return Content(HttpStatusCode.BadGateway, Common.Message.FORMAT_INVALID);
            ...
            //    return Content(HttpStatusCode.Forbidden, Common.Message.FORBIDDEN);
            List<string> Esign_GroupForms = new List<string>() { "A01_143_290922_VE", "A01_145_050919_VE", "A01_144_050919_VE" };
            if (Esign_GroupForms.Contains(formCode))
            {
                EsignBussiness esignbussiness = new EsignBussiness();
                // 5 khởi tạo
                Guid b_id = esignbussiness.ESignInt(formId, visitId, visitTypeCode, formCode, FillReportED(visit, formId, req_userName, formCode));
                bool is_esigned = false;
                // 6 ký luôn
                if (IsESignFromEMR(formId))
                {
                    is_esigned = esignbussiness.SignWithType6(b_id, req_userName, req_passWord, req_kind, formId, visitId, visitTypeCode);
                }
                else
                {
                    ...
                    {
                        bool is_esigned_type6 = esignbussiness.SignWithType6(b_id, req_userName, req_passWord, req_kind, formId, visitId, visitTypeCode);
                        if (is_esigned_type6)
                        {
                            bool is_esigned_type1 = esignbussiness.SignWithType1(b_id, req_kind);
                            if (is_esigned_type1)
                            {
                                is_esigned = true;
                            }
                    ...
                    if (formCode == "A01_143_290922_VE")
                    {
                        ...
                        if (is_esigned_type6)
                        {
                            bool is_esigned_type2 = esignbussiness.SignWithType2(b_id);
                            if (is_esigned_type2)
                            {
                                is_esigned = true;
                            }
                ...
                // 2 ký theo thứ tự setup trên eSign exp: A01_145_050919_VE 
                // 1 gửi mail
                //var type_flow = 6;
                //if ("A01_145_050919_VE" == formCode)
                //{
                //    type_flow = 2
                //}
                // lần đầu tiên ký
                if (is_esigned)
                {
                    ...
                    var confirm = unitOfWork.EIOFormConfirmRepository
                                  .FirstOrDefault(e => !e.IsDeleted && e.FormId == eio_form.Id
                                                        && e.ConfirmType != null && e.ConfirmType.ToUpper() == req_kind.ToUpper()
                                                        && e.Note == note
                                                     );

                    if (confirm != null) return Content(HttpStatusCode.BadRequest, Common.Message.INFO_INCORRECT);
                    EIOFormConfirm new_confirm = new EIOFormConfirm()
                    {
                        FormId = formId,
                        ConfirmType = req_kind,
                        ConfirmAt = DateTime.Now,
                        ConfirmBy = req_userName,
                        EsignId = b_id,
                    };
        ...
        private bool IsESignFromEMR(Guid formId)
        {
            return unitOfWork.EIOFormConfirmRepository.Count(e => !e.IsDeleted && e.FormId == formId) > 0;
        }
        ...
        private void LogLastUpdate(User user, Guid id, string formCode)
        {
            ...
            switch (formCode)
            {
                case "A03_045_290422_VE": //phiếu GDSK
                    form = unitOfWork.PatientAndFamilyEducationContentRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == id);
                    if (form == null)
                        return;
                    form.UpdatedInfo = user.FirstName + " (" + user.Username + ")";
                    unitOfWork.PatientAndFamilyEducationContentRepository.Update(form);
                    Guid? PatientAndFamilyEducationId = form.PatientAndFamilyEducationId;
                    var form_parent = unitOfWork.PatientAndFamilyEducationRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == PatientAndFamilyEducationId);
                    if (form_parent != null)
                        unitOfWork.PatientAndFamilyEducationRepository.Update(form_parent);
                    unitOfWork.Commit();
                    return;
                case "OPD_A02_007_080121_VE":// OPD ĐGBĐ thông thường
                    form = unitOfWork.OPDInitialAssessmentForShortTermRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == id);
                    if (form != null)
                    {
                    ...
                    return;
                case "EOC_A02_007_080121_VE":// EOC ĐGBĐ thông thường
                    form = unitOfWork.EOCInitialAssessmentForShortTermRepository.FirstOrDefault(e => !e.IsDeleted && e.Id == id);
                    if (form != null)
                    {
        ...
        private bool CheckAcctionByFormCode(Guid visitId, Guid formId, User user, string formCode)
        {
            ...
            switch (formCode)
            {
                ...
                case "A02_006_221221_V": // Đánh giá nguy cơ vi khuẩn đa kháng (MDRO)
                    if (visitType == "ED")
                    {
                        success = IsCheckPermission(user.Username, "APICFEDA02_006_221221_V");
                        if (success)
                            RemoveUnlock24h(visitId, formId);
                        return success;
                    }
                    if (visitType == "IPD")
                    {
                        success = IsCheckPermission(user.Username, "APICFIPDA02_006_221221_V");
                        if (success)
                            RemoveUnlock24h(visitId, formId);
                        return success;
                    }
                    if (visitType == "OPD")
                    {
                        success = IsCheckPermission(user.Username, "APICFOPDA02_006_221221_V");
                        if (success)
                            RemoveUnlock24h(visitId, formId);
                        return success;
                    }
                    if (visitType == "EOC")
                    {
                        success = IsCheckPermission(user.Username, "APICFEOCA02_006_221221_V");
                        if (success)
                            RemoveUnlock24h(visitId, formId);
                        return success;
                    }
