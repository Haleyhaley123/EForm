...
namespace EForm.BaseControllers
{
    public class BaseApiController : ApiController
    {
        ...
        private User GetUserDev(string ip)
        {
            ...
            if (ip == "10.115.88.136") return unitOfWork.UserRepository.FirstOrDefault(u => u.Username == "haulv4");
            if (ip == "10.115.88.74") return unitOfWork.UserRepository.FirstOrDefault(u => u.Username == "halt63");
            if (ip == "10.115.88.124") return unitOfWork.UserRepository.FirstOrDefault(u => u.Username == "tungpa1");
            if (ip == "::1")
            {
        ...
        protected bool hasAction(string action_code)
        {
            ...
            return (list_action != null && list_action.Contains(action_code)) || !IsProInv();
        }
        ...
        protected async Task<Customer> SyncHisCustomerAsync(string pid, Guid? customer_id = null)
        {
            var his_patient_info = new SyncOHPatientInfo(unitOfWork);
            return await his_patient_info.SyncCustomerAsync(pid, customer_id);
        }
        ...
        public string SaveFileEsign(Guid visitId, Guid formId, string data, string visitTypeCode)
        {
