...
namespace Clients.HisClient
{
    ...
    {
        public static async Task<JToken> GetScreenStaphylococcusAureus(string sidecode,string pid)
        {
            string url_postfix = string.Format("/LABCONN_EMR/1.0.0/getLabResultByPID_{0}?PID={1}", sidecode, pid);
            return await GetAsync(url_postfix, "DSXetNghiem", "XetNghiem");
        }
    ...
    public class VIHCClient
    {
        private static readonly string AuthToken = ConfigurationManager.AppSettings["VIHC_API_SERVER_TOKEN"] != null ? ConfigurationManager.AppSettings["VIHC_API_SERVER_TOKEN"].ToString() : "f130551d-52ca-4e6c-9dfe-526d8e6d7334";
        private static readonly string ViHCURL = ConfigurationManager.AppSettings["VIHC_API_SERVER_URL"] != null ? ConfigurationManager.AppSettings["VIHC_API_SERVER_URL"].ToString() : "http://10.115.40.179";
        public static async Task<JToken> GetMohFinalReport(dynamic data)
        {
            string url_postfix = string.Format("{0}/api/ApiReport/MohFinalReport", ViHCURL);
            return await ApiRequest.PostAsync(url_postfix, data, AuthToken);
        }
    }
}
